...
<div class="menu">
<p><strong>MENU:</strong> <a href="../mysql.php" target="_blank">Database (MySQL) Manager</a></p>
<p><strong>DISPLAY MODE:</strong> 100 rows/page (<a href="#">Click here to display ALL ROWS</a>)</p>
</div>
...
while($row = mysql_fetch_assoc($result)){
  ...
  echo '<button class="btn btn-mini tip-top btn-danger" title="Eliminate User from Database" onclick="eliminate_user('.$userID.')"><i class="icon-trash"></i></button>';
  $q_banned = mysql_query("SELECT * FROM ip_bans WHERE user_id='$userID'") or die(mysql_error());
  $check_banned = mysql_num_rows($q_banned);
  if ($check_banned == 1) {
    echo '<span id="btnAction-'.$userID.'"><button class="btn btn-mini tip-top btn-success" title="Unban User" onlick="unban_user('.$userID.')"><i class="icon-circle-blank"></i></button></span>';
  } else {
    echo '<span id="btnAction-'.$userID.'"><button class="btn btn-mini tip-top btn-inverse" title="Ban User from Entering IsharePortal" onlick="ban_user('.$userID.')" data-ip="'.$userIP.'"><i class="icon-ban-circle"></i></button></span>';
  }
  if ($checkSharerlink == 1) { echo '<button class="btn btn-mini tip-top btn-warning" title="Remove User Sharerlink" onclick="remove_sharerlink('.$userID.')"><i class="icon-remove"></i></button>'; }
  echo '</div>';
