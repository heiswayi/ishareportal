...
function prependNewShout() {
  $.ajax({
    type: 'GET', url: 'shoutbox_update.php?update=1&no_cache='+Math.random(),
    success: function(data){
      $("ul#chat").prepend(data).fadeIn();
      $("ul#chat li:last-child").last().remove();
    }
  });
}
/*
function loadNewShout(shoutno) {
  $.ajax({
    type: 'GET', url: 'shoutbox_newshout.php?load='+shoutno,
    success: function(data){
      $("#displayNewShoutBtn").html('').fadeOut();
      $("ul#chat").prepend(data).fadeIn();
      $("ul#chat li").slice(-shoutno).remove();
      displayNewShout = 0;
    }
  });
}
    ...
    success: function(data){
      if (initFirstCheck == true) {
        ...
        if (parseInt(data) > shoutCount) {
          shoutCount = parseInt(data);
          newShout = newShout + 1;
          prependNewShout(); // when use (displayNewShout), cancel auto-prepend new shout function
          
          //displayNewShout = displayNewShout + 1;
          //if (displayNewShout == 1) { $("#displayNewShoutBtn").html('<button class="btn btn-block btn-primary" type="button" onClick="loadNewShout('+displayNewShout+')">'+displayNewShout+' new shout. Click here to load.</button>').fadeIn(); }
          //else { $("#displayNewShoutBtn").html('<button class="btn btn-block btn-primary" type="button" onClick="loadNewShout('+displayNewShout+')">'+displayNewShout+' new shouts. Click here to load.</button>').fadeIn(); }
